...
- While a trajectory is shown, a slim legend bar at the foot of the map explains the speed colours: a continuous blue→red gradient whose scale stretches automatically to the speeds actually occurring in the current trajectory, with min / mid / max values (km/h) below the bar.
